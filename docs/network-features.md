...
- **端口扫描**：扫描统一发现端口 `4045/10429/5555/5025/111`
- **设备验证**：Pico TCP 必须通过版本握手；单纯端口开放只会作为待验证候选
- **响应时间测量**：显示每个设备的网络延迟

#### 发现结果与认证等级

网络发现只产生候选或框架级证据，不能直接提升为真实硬件支持：

- `mock`：端口开放、静态 profile 或单元测试，只能支持 `experimental`/`community` 口径。
- `fixture`：有协议 fixture 或离线回放，最高只能标记为 `fixture`。
- `hardware`：真实设备完成连接、采集、结果文件 hash 记录后，才允许标记 `verified` 或 `certified`。

所有真实硬件结论必须同步到 `docs/真实硬件认证矩阵.md` 和数据库的 `certificationEvidence.records`。
...
```json
{
  ...
  "logicAnalyzer.network.defaultPorts": [4045, 10429, 5555, 5025, 111],
...
- `defaultPorts`: 默认扫描端口列表。`4045` 是 Pico TCP 端口，`10429` 是 Saleae Logic 2 API，`5555/5025/111` 是常见 SCPI 入口。
