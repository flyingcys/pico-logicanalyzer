...
本文件描述的是目标架构和当前主要目录，但质量状态需按最新验证理解：

- `src` 下当前核心目录包括 `drivers`、`decoders`、`models`、`services`、`providers`、`frontend`、`webview`、`driver-sdk`、`database`、`tools`、`utils`。
- `npm run build` 可以完成打包，但 webpack 在扩展端和 webview 端均启用 `transpileOnly`，不会阻塞类型错误。
- `npm run typecheck` 当前失败，说明“类型安全”仍是目标状态，不是已达成状态。
- 详细问题和下一步见 [当前现状与下一步计划](current-status-and-next-steps-2026-04-27.md)。

### 架构分层图
```
┌─────────────────────────────────────────────────┐
│                用户界面层                         │
│     frontend/ (主路径) │ webview/ (兼容壳)      │
├─────────────────────────────────────────────────┤
│                业务服务层                         │
...
解码器测试当前统一放在项目根目录 `tests` 下，不在 `src/decoders/tests` 下新增测试。
...
- 内置性能优化和内存管理
- 可扩展的协议注册机制
- 当前内置协议集中在 I2C、SPI、UART，协议数量距离原 sigrok/Python 生态仍有明显差距。
...
### 4. frontend/ - 用户界面主路径
**职责**: 基于 Vue3 + Element Plus 的现代化用户界面主路径

**核心文件**:
- `app/App.vue` - 应用主组件
- `app/main-vscode.ts` - Webview 入口
- `app/main-html.ts` - HTML 入口
- `app/index.html` - HTML 模板

**子目录**:
- `app/components/` - Vue 组件库
...
### 5. webview/ - 兼容壳
**职责**: 为旧路径保留最小 shim / re-export，避免迁移期间直接打断既有引用
- Canvas 高性能波形渲染
- 响应式设计和主题系统
- 完整的国际化支持
...
当前 `src/commands` 目录为空，主要命令直接注册在 `src/extension.ts`。后续如果命令继续增长，应拆分到此目录。
