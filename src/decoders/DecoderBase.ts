...
export abstract class DecoderBase {
  ...
  protected wait(conditions: WaitCondition): WaitResult {
    ...
    // 如果是跳过条件，直接返回当前状态
    if (this.isSkipCondition(conditions)) {
      return {
    ...
    while (this.sampleIndex < maxSamples) {
      ...
      // 检查条件是否匹配
      const matched = this.checkConditions(conditions);
      if (matched.some(m => m)) {
        return {
          pins: this.getCurrentPins(),
          sampleNumber: this.sampleIndex,
  ...
  /**
   * 准备通道数据
   * @param channels 原始通道数据
   * @param selectedChannels 选中通道映射
   */
  protected prepareChannelData(
    channels: ChannelData[],
    selectedChannels: DecoderSelectedChannel[]
  ): void {
    this.channelData = [];
    ...
    for (let i = 0; i < this.channels.length; i++) {
      const selectedChannel = selectedChannels.find(sel => sel.decoderIndex === i);
      if (selectedChannel && selectedChannel.captureIndex < channels.length) {
        this.channelData[i] = channels[selectedChannel.captureIndex].samples;
      } else {
        // 如果通道未选择，创建空数据
        const maxLength = channels.length > 0 ? channels[0].samples.length : 0;
        this.channelData[i] = new Uint8Array(maxLength);
      }
    }
  }
}
