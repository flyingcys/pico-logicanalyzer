...
import { AnalyzerDriverType, CaptureError, TriggerType, CaptureMode } from '../models/Enums';
...
describe('AnalyzerDriverBase', () => {
    ...
    describe('采集模式管理', () => {
        test('getCaptureMode应该根据通道数正确返回模式', () => {
            // 测试8通道模式
    ...
    describe('采集功能测试', () => {
        test('startCapture应该成功启动采集', async () => {
        ...
        test('stopCapture应该成功停止采集', async () => {
    ...
    describe('设备信息管理', () => {
        test('getDeviceInfo应该返回完整的设备信息', () => {
            const deviceInfo = driver.getDeviceInfo();
            
            expect(deviceInfo).toBeDefined();
            expect(deviceInfo.name).toContain('Test Analyzer');
        ...
        test('enterBootloader应该成功执行', async () => {
    ...
    describe('网络功能测试', () => {
        test('sendNetworkConfig应该返回false对于非网络设备', async () => {
