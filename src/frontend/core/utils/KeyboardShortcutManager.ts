...
export type ShortcutCommandDispatcher = (
  command: string,
  payload?: unknown
...
export class KeyboardShortcutManager {
  ...
  private commandDispatcher: ShortcutCommandDispatcher | null | undefined;
  ...
  private dispatchCommand(command: string, payload?: unknown): unknown | Promise<unknown> {
    if (this.commandDispatcher === null) {
      return undefined;
    }

    if (this.commandDispatcher) {
      return this.commandDispatcher(command, payload);
    }

    return getDefaultHost().sendCommand(command, payload);
  }
   ...
   * 设置命令分发器
   */
  public setCommandDispatcher(dispatcher?: ShortcutCommandDispatcher | null) {
