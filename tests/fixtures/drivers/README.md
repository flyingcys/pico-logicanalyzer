...
| `pico-serial-non-contiguous-channel-frames.json` | 手工构造 8/16/24 通道模式二进制帧 | 非零起始、非连续通道必须按捕获通道数组下标取压缩 bit |

## 字段约定

- `frameHex`：可直接转为 `Buffer` 的十六进制串。
- `sampleCount`：帧头部的样本数量。
- `expectedChannelSamples`：以真实通道号为 key 的 0/1 样本序列。Pico 固件按请求通道顺序压缩每个样本 word，驱动拆通道时必须使用捕获通道数组下标。
