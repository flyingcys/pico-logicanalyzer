...
import { CaptureSession, AnalyzerChannel, BurstInfo } from '../../../src/models/CaptureModels';
...
describe('CaptureSession', () => {
    ...
    describe('基础属性测试', () => {
        it('应该有正确的默认值', () => {
            expect(session.frequency).toBe(1000000); // 1MHz默认
        ...
        it('totalSamples应该正确计算', () => {
            session.preTriggerSamples = 1000;
            session.postTriggerSamples = 10000;
            session.loopCount = 0;
            
            // 基于原版公式: postTriggerSamples * (loopCount + 1) + preTriggerSamples
            const expectedTotal = 10000 * (0 + 1) + 1000;
            expect(session.totalSamples).toBe(expectedTotal);
        });
        
        it('totalSamples应该在有loopCount时正确计算', () => {
            session.preTriggerSamples = 1000;
            session.postTriggerSamples = 5000;
            session.loopCount = 3;
            
            const expectedTotal = 5000 * (3 + 1) + 1000; // 20000 + 1000 = 21000
            expect(session.totalSamples).toBe(expectedTotal);
        });
    });
    ...
    describe('克隆功能测试', () => {
        ...
        it('cloneSettings应该只复制设置，不包含样本数据', () => {
            // 设置原始session
            session.frequency = 50000000;
            session.preTriggerSamples = 2000;
            session.postTriggerSamples = 20000;
    ...
    describe('边界条件测试', () => {
        ...
        it('应该处理极大样本数', () => {
            ...
            const expectedTotal = 1000000 * 11 + 100000;
            expect(session.totalSamples).toBe(expectedTotal);
        });
    ...
    describe('性能测试', () => {
        it('totalSamples计算应该高效', () => {
            const startTime = Date.now();
            
            for (let i = 1; i < 10000; i++) { // 从1开始避免0值
